 ...
 *    supported).
 * CSS is persisted in localStorage; the background media blob is persisted
 * in IndexedDB with its metadata in localStorage; the inspector flag is
...
#dsh-select-popup .dshSelectOpt[disabled]{opacity:.4;cursor:default}`;
		...
		const NS = "css-inject";
		const STORAGE_KEY = "dsh.user-css";
		const STYLE_ID = "dsh-user-css";
		const BG_META_KEY = "dsh.user-bg";
		const BG_STYLE_ID = "dsh-user-bg";
		const BG_VIDEO_ID = "dsh-bg-video";
		const DB_NAME = "dsh-user-bg";

		// ---- persisted CSS helpers ----
		...
		// ---- IndexedDB helpers for the background blob ----
		const idb = (() => {
			let dbPromise = null;
			...
			return {
				get: (key) => run("readonly", (store) => store.get(key)).catch(() => null),
				set: (key, value) => run("readwrite", (store) => store.put(value, key)).then(() => {}),
				del: (key) => run("readwrite", (store) => store.delete(key)).then(() => {})
			};
		})();
		...
		/** Read the persisted background metadata (never throws). */
		function readBgMeta() {
			try {
		...
		/** Write the persisted background metadata (never throws). */
		function writeBgMeta(meta) {
			try {
				if (meta === null) localStorage.removeItem(BG_META_KEY);
				else localStorage.setItem(BG_META_KEY, JSON.stringify(meta));
			} catch {}
		}
		...
		function restoreBackground() {
			...
			(async () => {
				try {
					...
					const blob = await idb.get("blob");
					if (meta === null || !(blob instanceof Blob)) return;
					const kind = meta.kind === "video" ? "video" : "image";
					const url = URL.createObjectURL(blob);
					const opacity = typeof meta.opacity === "number" ? meta.opacity : 60;
					bgStore.update({ kind, url, name: typeof meta.name === "string" ? meta.name : null, opacity });
		...
		function readInspFlag() {
			try { return localStorage.getItem(INSP_KEY) === "1"; } catch { return false; }
		}
		function writeInspFlag(on) {
			try { localStorage.setItem(INSP_KEY, on ? "1" : "0"); } catch {}
		}
		...
		const insp = (() => {
			...
			function ensure() {
				...
				headEl.addEventListener("mousedown", startDrag);
				try {
					const pos = JSON.parse(localStorage.getItem("dsh.inspector.pos") || "null");
			...
			function endDrag() {
				if (dragState !== null && panel !== null) {
					try {
						localStorage.setItem("dsh.inspector.pos", JSON.stringify({ left: panel.style.left, top: panel.style.top }));
					} catch {}
				}
				dragState = null;
		...
		const SEL_KEY = "dsh.select-beautify";
		const SEL_POPUP_ID = "dsh-select-popup";
		function readSelFlag() {
			try { return localStorage.getItem(SEL_KEY) === "1"; } catch { return false; }
		}
		function writeSelFlag(on) {
			try { localStorage.setItem(SEL_KEY, on ? "1" : "0"); } catch {}
		}
		const selBeautify = (() => {
			let on = false;
			let popup = null;
		 ...
		 * State lives in the `insp` singleton (localStorage-backed) so it keeps
		...
		function BackgroundSection({ t }) {
			...
			const onPick = async (event) => {
				...
				const previous = bgStore.getSnapshot();
				const url = URL.createObjectURL(file);
				try {
					await idb.set("blob", file);
					writeBgMeta({ kind: nextKind, name: file.name, opacity: bgStore.getSnapshot().opacity });
					applyBackground({ kind: nextKind, url }, bgStore.getSnapshot().opacity);
					bgStore.update({ kind: nextKind, url, name: file.name, opacity: bgStore.getSnapshot().opacity });
					if (previous.url !== null && previous.url !== url) URL.revokeObjectURL(previous.url);
					setStatus("saved");
				} catch {
			...
			const onOpacity = (event) => {
				...
				applyBackground(current, next);
				writeBgMeta({ kind: current.kind, name: current.name, opacity: next });
			};
			const remove = async () => {
				const current = bgStore.getSnapshot();
				applyBackground(null, current.opacity);
				bgStore.update({ kind: null, url: null, name: null, opacity: current.opacity });
				if (current.url !== null) URL.revokeObjectURL(current.url);
				try {
					writeBgMeta(null);
					await idb.del("blob");
					setStatus("cleared");
